...
function getReward() 
	local wall_c = -20
	local cp_c = checkpointValue
	local terrain_c = -20
	local time_c = -10
	local backwards_c = -2
	local start_c = -1000
	if data.race_state == 115 then 
		return wall_c * getWallPenalty() + cp_c * getCpReward() + terrain_c * getTerrainPenalty() 
			+ time_c * getTimePenalty() + backwards_c * getBackwardsPenalty() + start_c * getStartPenalty()
	end
	return 0
end

mushroomUsed = false
function getStartPenalty() 
	if not mushroomUsed then
		if data.time < 900 and data.itemSlot ~= 90 then 
			mushroomUsed = true
			return 1
		end
	end
